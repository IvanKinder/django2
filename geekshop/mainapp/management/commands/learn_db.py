...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        action_1_price = When(action_1_condition, then=F('product__price') * F('quantity') * (1 - action_1_discount))
        action_2_price = When(action_2_condition, then=F('product__price') * F('quantity') * (1 - action_2_discount))
        action_expired_price = When(action_expired_condition, then=F('product__price') * F('quantity') * (1 - action_3_discount))
